# -*- coding: utf-8 -*-
"""
SFMC → AJO HTML translator with a robust HTML pre-clean step and structured Excel logging.

Pipeline:
  1) Pre-clean HTML (preserve AMPScript, normalize encoding, fix tags, indent).
  2) Convert AMPScript IF/ELSE blocks to AJO Liquid ({% if %}...{%/if%}).
  3) Apply table-driven replacements (de-para).
  4) Replace %%=v(@var)=%% → {{ expr }} ONLY for mapped variables.
  5) Safely comment any remaining AMPScript (attribute-safe), hoisting {% let ... %}.
  6) Produce a neat Excel log (Summary, Variables, Unmapped_Variables, Commented_AMPScript).
...
# --- HTML libraries for pre-clean ---
try:
    from bs4 import BeautifulSoup
except ImportError:
    raise SystemExit(
        "\nMissing dependency: 'beautifulsoup4'\n"
        "Install:  py -m pip install beautifulsoup4\n"
    )

try:
    import html5lib  # noqa: F401 (presence ensures parser availability)
except ImportError:
    raise SystemExit(
        "\nMissing dependency: 'html5lib'\n"
        "Install:  py -m pip install html5lib\n"
    )
...
# Pre-clean knobs (keep simple; could be made CLI flags if needed)
PRECLEAN_INDENT = 2       # indentation width when serializing
PRECLEAN_USE_FTFY = True  # normalize broken unicode when ftfy is available
...
# =========================================
# Pre-clean: invisible chars, utf-8, tags
# =========================================
AMP_TOKEN_RE = re.compile(r"%%.*?%%", re.DOTALL)  # tokenizes both %%=...=%% and %%[ ... ]%% blocks
ZERO_WIDTH_RE = re.compile(r"[\u200B-\u200D\u2060\uFEFF]")
CTRL_EXCEPT_WS_RE = re.compile(r"[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]")
...
def decode_robust(raw: bytes) -> str:
    """
    Decode bytes robustly, preferring UTF-8 (with/without BOM), then latin-1 as a last resort.
    We always return str and later write as UTF-8.
    """
    for enc in ("utf-8-sig", "utf-8", "latin-1"):
        try:
            ...
            continue
    # Fallback: decode as UTF-8 replacing errors
    return raw.decode("utf-8", errors="replace")


def strip_invisible(text: str) -> str:
    """
    Remove zero-width and control characters (except whitespace) that often break parsers.
    """
    text = ZERO_WIDTH_RE.sub("", text)
    text = CTRL_EXCEPT_WS_RE.sub("", text)
    return text


def tokenize_ampscript(html: str):
    """
    Temporarily replace all AMPScript blocks/tokens by opaque placeholders so that
    the HTML parser/serializer doesn't try to "fix" them.
    """
    mapping = {}
    def repl(m):
        idx = len(mapping) + 1
        token = f"__AMPSCRIPT_TOKEN_{idx:04d}__"
        mapping[token] = m.group(0)
        return token
    return AMP_TOKEN_RE.sub(repl, html), mapping


def detokenize_ampscript(html: str, mapping: dict) -> str:
    """Restore original AMPScript in place of placeholders."""
    for token, original in mapping.items():
        html = html.replace(token, original)
    return html


def html_pretty_format_preserving_ampscript(raw_html: str, indent_width: int = 2, use_ftfy: bool = True) -> str:
    """
    Clean/repair/indent HTML while preserving AMPScript verbatim.
      - Fix broken Unicode (ftfy) if available.
      - Strip invisible/control characters.
      - Tokenize AMPScript.
      - Parse with html5lib (tolerant) and serialize with BeautifulSoup.
      - Normalize indentation and squeeze excessive blank lines.
      - Detokenize AMPScript.
    """
    text = raw_html
    if HAS_FTFY and use_ftfy:
        text = fix_text(text)

    text = strip_invisible(text)

    tokenized, mapping = tokenize_ampscript(text)

    # html5lib repairs/normalizes markup; BeautifulSoup handles serialization
    soup = BeautifulSoup(tokenized, "html5lib")
    formatted = soup.decode(formatter="html")

    # Normalize indentation width (BeautifulSoup defaults to 2 spaces)
    if indent_width != 2:
        formatted = re.sub(
            r"^( +)",
            lambda m: " " * (len(m.group(1)) // 2 * indent_width),
            formatted,
            flags=re.MULTILINE
        )

    # Squeeze extra blank lines without being aggressive
    formatted = re.sub(r"\n{3,}", "\n\n", formatted)

    restored = detokenize_ampscript(formatted, mapping)
    # Always use \n newlines (Windows readers handle it fine; consistent for AJO)
...
def write_excel_log(commented_blocks, total_found, total_replaced, output_path, warnings, covered_vars, var_expr,
    ...
    rows = [
        ...
        ("Pre-clean (indent)", str(preclean_info.get("indent"))),
        ("Pre-clean (ftfy)", "yes" if preclean_info.get("ftfy") else "no"),
...
# 1) Read and PRE-CLEAN HTML (preserving AMPScript)
raw_bytes = read_bytes(html_input)
raw_text  = decode_robust(raw_bytes)                 # robust decoding → str
cleaned   = html_pretty_format_preserving_ampscript( # repair/indent while preserving AMPScript
    raw_text,
    indent_width=PRECLEAN_INDENT,
    use_ftfy=PRECLEAN_USE_FTFY
)
# Use the cleaned HTML for the rest of the pipeline
html = cleaned
...
# 7) Save final HTML (UTF-8, LF newlines)
Path(html_output).write_text(final_html.replace("\r\n", "\n"), encoding="utf-8")
...
log_xlsx = write_excel_log(
    ...
    var_expr=var_expr,
    preclean_info={"indent": PRECLEAN_INDENT, "ftfy": (HAS_FTFY and PRECLEAN_USE_FTFY)}
)
...
print(f"Pre-clean: indent={PRECLEAN_INDENT}, ftfy={'yes' if (HAS_FTFY and PRECLEAN_USE_FTFY) else 'no'}")
